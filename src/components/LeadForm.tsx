...
export default function LeadForm({ variant = "sidebar", className = "" }: LeadFormProps) {
  ...
  return (
    <div className={`${containerBaseClass} ${className}`}>
      {/* Header */}
      <div className={`flex ${isLarge ? 'flex-col items-center text-center mb-10' : 'items-center gap-2.5 mb-5'}`}>
        <div className={`flex items-center ${isLarge ? 'gap-5 mb-4' : 'gap-2.5'}`}>
          ...
          <div>
            <p className={`${isLarge ? 'text-3xl md:text-4xl' : 'text-sm'} font-bold text-brand-ink leading-tight`}>
              Talk to a Licensed Advisor
            </p>
            <p className={`${isLarge ? 'text-base mt-2' : 'text-[11px]'} text-brand-muted`}>
      ...
      {status === "success" ? (
        ...
        <form onSubmit={handleSubmit} className={`${isLarge ? 'grid grid-cols-1 md:grid-cols-2 gap-6' : 'flex flex-col gap-3'}`}>
          <div className={`${isLarge ? 'col-span-1' : ''}`}>
            <label htmlFor={`lead-name-${variant}`} className={`${isLarge ? 'text-sm mb-2.5' : 'text-[11px] mb-1'} font-semibold text-brand-muted uppercase tracking-wide block`}>
              Your Name
            </label>
            <input
              ...
              required
              className={`w-full rounded-xl border border-gray-200 ${isLarge ? 'px-5 py-4 text-lg' : 'px-3 py-2.5 text-sm'} text-brand-ink placeholder:text-gray-400 focus:outline-none focus:border-brand-green focus:ring-2 focus:ring-brand-green/15 transition-all`}
            />
          </div>

          <div className={`${isLarge ? 'col-span-1' : ''}`}>
            <label htmlFor={`lead-phone-${variant}`} className={`${isLarge ? 'text-sm mb-2.5' : 'text-[11px] mb-1'} font-semibold text-brand-muted uppercase tracking-wide block`}>
              Phone Number
            </label>
            <input
          ...
          <div className={`${isLarge ? 'col-span-1' : ''}`}>
            <label htmlFor={`lead-topic-${variant}`} className={`${isLarge ? 'text-sm mb-2.5' : 'text-[11px] mb-1'} font-semibold text-brand-muted uppercase tracking-wide block`}>
              Topic
            </label>
            <select
              ...
              required
              className={`w-full rounded-xl border border-gray-200 ${isLarge ? 'px-5 py-4 text-lg' : 'px-3 py-2.5 text-sm'} text-brand-ink bg-white focus:outline-none focus:border-brand-green focus:ring-2 focus:ring-brand-green/15 transition-all`}
            >
          ...
          <div className={`${isLarge ? 'col-span-1 flex items-end' : 'w-full'}`}>
            <button
              type="submit"
              disabled={status === "loading"}
              className={`btn-primary w-full ${isLarge ? 'py-5 text-lg rounded-xl' : 'text-sm mt-1'} disabled:opacity-60 disabled:cursor-not-allowed`}
            >
      ...
      {status === "error" && (
        <p className={`text-red-500 mt-2 ${isLarge ? 'text-sm text-center' : 'text-xs'}`}>
          Something went wrong. Please try again.
        </p>
      )}
      {isLarge && status !== "success" && (
        <p className="text-base text-brand-muted text-center mt-8">
          Licensed advisors · 100% free · No commissions
        </p>
      )}
    </div>
